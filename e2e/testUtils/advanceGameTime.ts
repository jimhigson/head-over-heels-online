 ...
 * test waits for the world to do has to be stepped from here, even a zero-delta
 * timestep.
 */
 ...
 *
 * The condition is written in the spec but runs in the browser, so it reads
 * page globals (`window._e2e_store` and friends) directly and closes over
 * nothing: only its text crosses, never its scope. Anything from the spec's own
 * scope has to travel as `arg`, which crosses as data
 */
export const advanceUntilConditionMetInsidePage = <Arg = undefined>(
  page: Page,
  conditionMet: (arg: Arg) => boolean | Promise<boolean>,
  {
    arg,
    stepMs = 250,
    maxMs = 30_000,
  }: { arg?: Arg; stepMs?: number; maxMs?: number } = {},
): Promise<boolean> =>
  page.evaluate(
    async ({ conditionMetText, arg, stepMs, polls }) => {
      const ready = new Function(`return (${conditionMetText})`)() as (
        arg: unknown,
    ...
    {
      conditionMetText: conditionMet.toString(),
      arg: arg as Arg,
      stepMs,
      polls: Math.ceil(maxMs / Math.max(stepMs, 16)),
    },
  );
