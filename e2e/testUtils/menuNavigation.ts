...
import { spriteOptionValues } from "../../src/sprites/spritesheet/spritesheetData/spritesheetMetaData";
import { advanceUntil, whilePainting } from "./advanceGameTime";
...
export const setSkinViaMenus = async (
  ...
  // the menu item cycles through the options it offers, so clicking it once per
  // option is enough to reach any of them from wherever it currently sits:
  for (let clicks = 0; clicks < spriteOptionValues.length; clicks++) {
 ...
 *
 * Each step waits for the dialog to actually change before the next, so this is
 * popping a stack of known depth rather than clicking hopefully: it only goes
 * round again because there is still a submenu above the main menu, never
 * because a click might not have worked. If backing out stops making progress
 * it throws, rather than returning quietly and leaving the caller to fail
 * somewhere else with an error that does not name this as the cause.
 */
export const backToMainMenu = async (page: Page, projectName: string) => {
  const formattedName = formatProjectName(projectName);
  const mainMenu = page.locator('[data-dialog-id="mainMenu"]');
  const seenDialogIds: DialogId[] = [];

  while (!(await mainMenu.isVisible().catch(() => false))) {
    const dialogId = await getCurrentDialogId(page);
    if (dialogId === null) {
      throw new Error(
        `no dialog is open, so there is nothing to back out of, but the main menu never appeared (backed out of: ${seenDialogIds.join(" → ") || "nothing"})`,
      );
    }
    if (seenDialogIds.includes(dialogId)) {
      throw new Error(
        `backing out returned to the "${dialogId}" dialog instead of reaching the main menu (backed out of: ${seenDialogIds.join(" → ")})`,
      );
    }
    seenDialogIds.push(dialogId);
...
/**
 * wait for a dialog the *world* has to open - collecting a crown, walking onto
 * a scroll - by stepping game time until it is there. {@link waitForDialog}
 * only paints, which is right for a dialog the app opens on its own but leaves
 * a frozen world waiting for something that will never happen
 */
export const advanceUntilDialog = async (
  page: Page,
  dialogId: DialogId | string,
): Promise<void> => {
  const dialog = page.locator(`[data-dialog-id="${dialogId}"]`);
  const opened = await advanceUntil(page, () =>
    dialog.isVisible().catch(() => false),
  );
  if (!opened) {
    throw new Error(
      `timed out advancing until the "${dialogId}" dialog opened`,
    );
  }
};
...
export const exitCrownsDialog = async (page: Page, logHeader: string) => {
  await test.step("Exit crowns dialog", async () => {
    ...
    await whilePainting(
      page,
      crownsDialog.waitFor({ state: "visible", timeout: longTimeout }),
    );
    ...
    await whilePainting(
      page,
      crownsDialog.waitFor({ state: "detached", timeout: longTimeout }),
    );
  });
};
