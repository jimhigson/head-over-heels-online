...
/**
 * Turn upscaled sprites on.
 *
 * Not `enableUpscaledSprites`, which waits for a pixi sprite to be drawing from
 ...
 * for below.
 */
const setUpscaledSprites = (page: Page) =>
...
test.describe("Main menu at each browser's own size", () => {
  ...
  test("Main menu at the device's own viewport", async ({ page }, testInfo) => {
    ...
    await test.step("turn upscaled sprites on", async () => {
      await setUpscaledSprites(page);
