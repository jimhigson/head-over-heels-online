...
import {
  ...
  upscaledSpritesHudTextMaxDiffPixelRatio,
} from "./testUtils/screenshots";
import { test } from "./testUtils/test";

// the upscaled captures have baselines for these two projects only, matching
// where the camera-rotation snapshots run
const projectNames = ["chromium-desktop", "mobile-safari"];
test.beforeEach(() => {
  const { name } = test.info().project;
  test.skip(
    !projectNames.includes(name),
    `upscaled-sprite snapshots are not browser-specific; run only on ${projectNames.join(", ")}`,
  );
});
...
test.describe("upscaled sprites", () => {
  // the upscale shader is slow to compile where rendering is in software
  ...
  test("turning it on live upscales the hud, not only the room", async ({
    page,
  }, testInfo) => {
    ...
    await enableUpscaledSprites(page);
    ...
    await expect(page).toHaveScreenshot(
      "upscaled-sprites-hud-live-toggle.png",
      {
        ...roomScreenshotOptions(testInfo.project.name),
        maxDiffPixels: undefined,
        maxDiffPixelRatio: upscaledSpritesHudTextMaxDiffPixelRatio,
      },
    );
  });
  ...
  test("opening a display menu does not rebuild the hud every frame", async ({
    ...
    // and at this baked upscale factor every rebuild re-rasterises the hud's text into
  ...
  test("the game starts when it is already on at boot", async ({
    ...
    // first frame is rendered upscaled rather than transitioning into it:
    await page.reload();
    ...
    await expect(page).toHaveScreenshot(
      "upscaled-sprites-boot-after-reload.png",
      {
        ...roomScreenshotOptions(testInfo.project.name),
        maxDiffPixels: undefined,
        maxDiffPixelRatio: upscaledSpritesHudTextMaxDiffPixelRatio,
