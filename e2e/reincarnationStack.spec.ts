...
import { advanceUntilTrue, whilePainting } from "./testUtils/advanceGameTime";
import {
  clickCheat,
  dispatchKeyPress,
  switchCharacter,
...
const readRoomsExploredCount = async (
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // wait for this dialog to actually close before popping the next, painting
    // while waiting so the menu keeps getting the ticks that close it:
    await whilePainting(
      page,
      page
  ...
  test("eating fish in A → B → C as different characters with extra lives, then reincarnating pops back through C → B → A restoring character/room/lives", async ({
      ...
      await advanceUntilTrue(
        ...
        () =>
          page.evaluate((roomId) => {
            ...
            return (
              state?.characterRooms[state.currentCharacterName]?.id === roomId
            );
          }, "egyptus1"),
        "the room to become egyptus1",
      );
      ...
      await advanceUntilTrue(
        ...
        () =>
          page.evaluate((before) => {
            const st = window.__e2e_currentPlayable?.()?.state;
            const lives =
              st !== undefined && "lives" in st ? st.lives : undefined;
            return typeof lives === "number" && typeof before === "number" ?
                lives > before
              : lives !== before;
          }, livesBefore),
        "the extra life to be collected",
      );
    ...
    await test.step("Swop to heels, go to room C (penitentiary1), summon extra life, eat fish", async () => {
      ...
      await advanceUntilTrue(
        ...
        "the room to become penitentiary1",
      );
