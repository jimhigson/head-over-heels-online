...
/**
 * rendered in just the 2 most important browsers so we don't have too many
 * screenshots
 */
const restrictToCameraRotationProjects = (): void => {
...
const scenarios: readonly SweepScenario[] = [
  ...
  // the smooth-sprites (upscaled) rendering across three sceneries
 ...
 * | emulatedResolution | "$$default"           | `-<resolutionName>`            |
 * | upscaledSprites      | false                 | `-upscaled`                      |
...
for (const scenario of scenarios) {
  test(`camera angles render deterministically: ${scenarioBaseName(scenario)} (${scenarioAxes(scenario)})`, async ({
    ...
    test.setTimeout(scenario.upscaledSprites ? 600_000 : 360_000);
    ...
    for (const spriteOption of spriteOptions) {
      ...
      for (const captureTimeMs of scenario.captureTimesMs ?? [0]) {
        ...
        for (const capture of capturesForScenario(scenario)) {
          ...
          const heldAfterId = await snapshotE2eCursor(page);
