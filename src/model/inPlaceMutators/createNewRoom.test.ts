...
import { roomJsonMatchers } from "../json/__test__/roomJsonMatchers";
...
describe("createNewRoom", () => {
  ...
  it("should create two width-adjacent chunks and remove the wall between them", () => {
    const gridPositions = [
      { x: 0, y: 0 },
      { x: 1, y: 0 },
    ];
    ...
    // Should have NO left/right walls at x=8 (the boundary between chunks)
    // The first chunk's left wall and second chunk's right wall should be removed

    // Check towards wall - should span both chunks (16 units)
    expect(wallsByDirection.towards?.length).toBe(1);
    expect(room).toHaveWall({
      direction: "towards",
      position: { x: 0, y: 0, z: 0 },
    ...
    // Check away wall - should span both chunks (16 units)
    expect(wallsByDirection.away?.length).toBe(1);
    expect(room).toHaveWall({
      direction: "away",
      position: { x: 0, y: 8, z: 0 },
    ...
    // Check left wall - should be at right edge of second chunk only
    expect(wallsByDirection.left?.length).toBe(1);
    expect(room).toHaveWall({
      direction: "left",
      position: { x: 16, y: 0, z: 0 },
    ...
    // Check right wall - should be at left edge of first chunk only
    expect(wallsByDirection.right?.length).toBe(1);
    expect(room).toHaveWall({
      direction: "right",
      position: { x: 0, y: 0, z: 0 },
  ...
  it("should handle non-zero room origin", () => {
    const gridPositions = [
      { x: 0, y: 0 },
      { x: 0, y: 1 },
    ];
    ...
    // Should have one consolidated floor spanning both chunks, offset by origin
    expect(room).toHaveFloor({
    ...
    // Should have NO away wall at y=12 (the boundary between chunks)
    // The first chunk's away wall and second chunk's towards wall should be removed
