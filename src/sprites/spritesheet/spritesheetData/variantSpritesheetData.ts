...
export type DoorHueSuffix = `hue=${ZxSpectrumRoomHue}`;
export const doorHueSuffixes = [
  ...
  "hue=white",
  "hue=yellow",
] as const satisfies readonly DoorHueSuffix[];
/** the destination hue a door-frame variant suffix recolours to */
export const doorHueOfSuffix = (suffix: DoorHueSuffix): ZxSpectrumRoomHue =>
  suffix.slice("hue=".length) as ZxSpectrumRoomHue;

/** the four on-screen action buttons, named by action not colour */
export type ArcadeButtonAction = "carry" | "carryAndJump" | "fire" | "jump";
export type ArcadeButtonSuffix = `action=${ArcadeButtonAction}`;
export const arcadeButtonSuffixes = [
  "action=carry",
  "action=carryAndJump",
  "action=fire",
  "action=jump",
] as const satisfies readonly ArcadeButtonSuffix[];
/** the button action an arcade-button variant suffix colours for */
export const arcadeButtonActionOfSuffix = (
  suffix: ArcadeButtonSuffix,
): ArcadeButtonAction => suffix.slice("action=".length) as ArcadeButtonAction;
