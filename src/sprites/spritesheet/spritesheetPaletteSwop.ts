import type { Color, Filter, Renderer as PixiRenderer } from "pixi.js";
...
const dimSwops: SpritesheetTextureSwops = {
 ...
 */
export const applyDimToSpritesheet = (
  pixiRenderer: PixiRenderer,
  baseSpritesheet: AppSpritesheet,
): AppSpritesheet => {
  const baseTexture = Texture.from(baseSpritesheet.textureSource);
  const dimmed = createSpritesheetVariant(pixiRenderer, dimSwops, baseTexture);
  baseTexture.destroy();
