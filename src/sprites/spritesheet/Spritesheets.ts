...
import { type bakeUpscaledTexture } from "../../game/render/filters/upscale/bakeUpscaledTexture";
import { importBakeUpscaledTextureOnce } from "../../game/render/filters/upscale/bakeUpscaledTexture.import";
...
export class Spritesheets {
   ...
   * (cpu-side only) so the original can be re-baked at a different baked
   * upscale factor without re-fetching. undefined when running under node
   * (Texture.EMPTY)
   */
  #sourceImage: HTMLImageElement | undefined;
  /**
   * the upscale factor baked into {@link #originalSpritesheet} (and
   * so inherited by the room sheet baked from it). 1 = the plain unprocessed
   * sheet
   */
  #bakedUpscaleFactor = 1;
  /**
   * the upscale bake, once {@link loadBakeModule} has fetched it. Only a
   * factor above 1 needs it, so it stays out of the graph of a player who
   * never turns smoothing on
   */
  #bakeUpscaledTexture: typeof bakeUpscaledTexture | undefined;
  /**
  ...
  /**
   * where each frame's art sits in {@link #originalSpritesheet}, when the
   * sheet it was baked from was composed for the upscale rather than taken
   * straight from the authored image
   */
  #frameOverrides: ReadonlyMap<string, FrameRect> | undefined;

  isTextureLoaded(spriteOptionName: SpriteOptionName): boolean {
    return this.#spriteOptionName === spriteOptionName;
  }

  get bakedUpscaleFactor(): number {
    return this.#bakedUpscaleFactor;
  }

  /**
   * whether {@link loadBakeModule} has resolved, so a sheet can be baked at a
   * factor above 1
   */
  get isBakeModuleLoaded(): boolean {
    return this.#bakeUpscaledTexture !== undefined;
  }
   ...
   */
  async loadBakeModule(): Promise<void> {
    ({ bakeUpscaledTexture: this.#bakeUpscaledTexture } =
      await importBakeUpscaledTextureOnce());
  }
   ...
   * `bakedUpscaleFactor` times its logical size) before the shadow preprocess bakes
   * over it
   */
  #rebakeOriginalAtFactor(
    ...
    if (
      import.meta.env.DEV &&
      bakedUpscaleFactor !== 1 &&
      this.#bakeUpscaledTexture === undefined
    ) {
      throw new Error(
        "cannot bake above 1x before loadBakeModule() has resolved",
      );
    }
    ...
    const repacked =
      bakedUpscaleFactor === 1 ? undefined : (
        composeUpscaleSheet(
          pixiRenderer,
          rawTexture,
    ...
    const sheetToBake = repacked?.texture ?? rawTexture;
    const baseTexture =
      bakedUpscaleFactor === 1 ? rawTexture : (
        this.#bakeUpscaledTexture!(
          pixiRenderer,
          sheetToBake,
          bakedUpscaleFactor,
        )
    ...
    this.#bakedUpscaleFactor = bakedUpscaleFactor;
    this.#frameOverrides = repacked?.frameOverrides;
  }
  ...
  rebuild(
    ...
    const originalRebuilt =
      ...
      bakedUpscaleFactor !== this.#bakedUpscaleFactor;
    if (originalRebuilt) {
      this.#rebakeOriginalAtFactor(pixiRenderer, bakedUpscaleFactor);
    }
  ...
  async loadImage(
    ...
    try {
      ...
      this.#bakedUpscaleFactor = 1;
      this.#frameOverrides = undefined;
  ...
  #buildOriginal(
    ...
    const spriteSheetData: AppSpritesheetDataWithVariants =
      frameOverrides === undefined ? rawSheetData : (
        {
          ...rawSheetData,
          frames: withRepackedRects(rawSheetData.frames, frameOverrides),
        }
      );
