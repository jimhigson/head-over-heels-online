...
const shaftWidth = blockSizePx.x * 0.5;
...
const slotToScreen = (slot: Xy): Xy =>
  addXy(projectWorldXyzToScreenXy(scaleXy(slot, half)), { x: 0, y: -3 - 16 });
...
const pointsToPath = (points: Xy[]): string =>
...
const arrowPath = (
  ...
  const perp: Xy = { x: -dir.y, y: dir.x };
  const shaftHalf = shaftWidth / 2;
  const headHalf = arrowSize / 2;
  const headStep = scaleXy(dir, arrowSize);
  const toBase = subXy(to, headStep);
  const alongPerp = (point: Xy, amount: number) =>
    addXy(point, scaleXy(perp, amount));

  if (doubleHeaded) {
    const fromBase = addXy(from, headStep);
    return pointsToPath([
      from,
      alongPerp(fromBase, headHalf),
      alongPerp(fromBase, shaftHalf),
      alongPerp(toBase, shaftHalf),
      alongPerp(toBase, headHalf),
      to,
      alongPerp(toBase, -headHalf),
      alongPerp(toBase, -shaftHalf),
      alongPerp(fromBase, -shaftHalf),
...
const EditorMapTeleporterLinkDecorator = ({
  ...
  const arrows = teleporterLinks.flatMap((link, index) => {
    ...
    const p1 = anchorFor(fromKey, link.from.itemId) ?? centre;
    const p2 = addXy(delta, anchorFor(toKey, link.to.itemId) ?? centre);
