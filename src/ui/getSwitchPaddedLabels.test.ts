...
const replaceSpaceWithFullWidth = (s: string) => s.replace(/ /g, "\u2003");
...
test("pads labels sliding from left to right for 4 options", () => {
  ...
  expect(result).toEqual(
    ["1    ", " 1.2 ", "  1.5", "    2"]
      // Replace spaces with full-width spaces for comparison
      .map(replaceSpaceWithFullWidth),
  );
});
...
test("pads labels sliding from left to right for 4 options with x suffix", () => {
  ...
  expect(result).toEqual(
    ["1x    ", " 1.2x ", "  1.5x", "    2x"].map(replaceSpaceWithFullWidth),
  ); // Replace spaces with full-width spaces for comparison
});
...
test("pads labels for 2 options", () => {
  ...
  expect(result).toEqual(["ON  ", " OFF"].map(replaceSpaceWithFullWidth));
});
...
test("pads labels for 3 single-char options", () => {
  ...
  expect(result).toEqual(["A  ", " B ", "  C"].map(replaceSpaceWithFullWidth));
});
...
test("pads labels for 3 options of unequal length", () => {
  ...
  expect(result).toEqual(
    ["4   ", " 8  ", "  AL"].map(replaceSpaceWithFullWidth),
  );
});
...
test("pads labels for 3 options of equal length", () => {
  ...
  expect(result).toEqual(
    ["4W  ", " 8W ", "  AL"].map(replaceSpaceWithFullWidth),
  );
});
...
test("centers single option", () => {
  ...
  expect(result).toEqual(["ONLY"].map(replaceSpaceWithFullWidth));
});
