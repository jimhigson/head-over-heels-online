...
import fragment from "./color-replace.frag?raw";
import source from "./color-replace.wgsl?raw";
 ...
 *
 */
export class CustomisedColorReplaceFilter extends Filter {
  ...
  constructor(
    ...args:
      ...
      | [DeprecatedColor?, DeprecatedColor?, number?]
  ) {
    let options = args[0] ?? {};
    ...
    if (
      ...
      options instanceof Float32Array
    ) {
      deprecation(
        "6.0.0",
    ...
    options = { ...CustomisedColorReplaceFilter.DEFAULT_OPTIONS, ...options };
   ...
   * @default 0x000000
   * @see CustomisedColorReplaceFilter#targetColor
   */
   ...
   * Tolerance/sensitivity of the floating-point comparison between colors (lower = more exact, higher = more inclusive)
   * @default 0.4
   * @see CustomisedColorReplaceFilter#tolerance
   */
