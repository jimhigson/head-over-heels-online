      ...
      (playableLosingLifeName) => {
        ...
        describe(`and it is ${playableLosingLifeName}'s last life`, () => {
          ...
          test(`and ${otherName} also has zero lives`, () => {
            ...
            expect(
              h.selectPlayable(playableLosingLifeName),
            ).not.toBeUndefined();
            h.playableLosesLife(
              playableLosingLifeName,
              originalGameStartingLives,
    ...
    describe("after losing lives, both characters have lives left", () => {
      ...
      */
        ...
        characters rejoin joined and at that door to avoid restarting with colliding individuals`, () => {
          ...
          expect(
            selectAbilities(h.gameState, "heels")!.carrying,
          ).not.toBeNull();
...
/**
 * REVISED LIVES MODEL - avoids the game being unplayable with only one character having lives left
 * by them swopping 2 lives for 1 in the player who just lost one
 */
describe("revised lives model", () => {
  describe("while not in symbiosis", () => {
    describe.each(individualCharacterNames)(
      "%s loses life",
      (playableLosingLifeName) => {
        ...
        describe(`and it is ${playableLosingLifeName}'s last life`, () => {
          test(`but ${otherName} still has >=2 lives left`, () => {
            const h = mutatorsTestHarness();

            // current guy has one:
            h.selectPlayable(playableLosingLifeName)!.state.lives = 1;
            // other guy has plenty:
            h.selectPlayable(otherName)!.state.lives = 4;

            h.playableLosesLife(playableLosingLifeName);

            // we got one lift back:
            expect(h.selectPlayable(playableLosingLifeName)!.state.lives).toBe(
              1,
            );
            expect(h.selectPlayable(otherName)!.state.lives).toBe(2);
            expect(h.gameState.currentCharacterName).toEqual(
              playableLosingLifeName,
            );
            h.expectGameContinues();
          ...
          test(`and ${otherName} also has <= 2 lives, game ends for ${playableLosingLifeName}`, () => {
            const h = mutatorsTestHarness();
            ...
            h.selectPlayable(playableLosingLifeName)!.state.lives = 1;
            // other guy has plenty:
            h.selectPlayable(otherName)!.state.lives = 2;
