...
const QuitGameHint = () => {
  ...
  switch (focussedItemId) {
    case "no":
      return <span class="text-single-line">Back to the game</span>;
    case "reincarnate":
      return (
        <BlockyMarkdown
          class={hintClassName}
    ...
    <DialogPortal>
      ...
      >
        <div class="flex flex-col gap-y-2 mt-1 items-center h-full">
          <span class="text-double-height resHandheld:mt-0 text-midRed zx:text-zxWhite toppy:text-toppyWarm1">
            End the Game?
          </span>
          <MenuItems class="text-lightGrey zx:text-zxWhite toppy:text-toppyGrey1 resHandheld:mt-0 resHandheld:!gap-y-1">
            <MenuItem
              doubleHeightWhenFocussed
              id="no"
              label="Back"
              class="selectedMenuItem:text-moss zx:selectedMenuItem:text-zxGreen toppy:selectedMenuItem:text-toppyCool2"
              leader={
                <span
                  class={`${spriteLeaderClasses} sprite ${noUnselectedTextureClass} ${noSelectedTextureClass} sprites-normal-height zx:sprite-revert-to-white`}
                />
              }
            ...
            {hasReincarnationPoint && (
              <MenuItem
                ...
                id="reincarnate"
                label="Reincarnate"
                class="selectedMenuItem:text-metallicBlue zx:selectedMenuItem:text-zxBlue toppy:selectedMenuItem:text-toppyCool3"
                leader={
                  <span
                    class={`${spriteLeaderClasses} sprite ${"texture-fish_1" satisfies TextureTailwindClass} ${"selectedMenuItem:texture-animated-fish" satisfies AnimatedTextureTailwindClass} sprites-normal-height zx:sprite-revert-to-white`}
                  />
                }
            ...
            <MenuItem
              ...
              id="yes"
              label="End game"
              class="selectedMenuItem:text-midRed zx:selectedMenuItem:text-zxYellow toppy:selectedMenuItem:text-toppyPink2"
              onSelect={useDispatchActionCallback(gameOver, {
                reincarnationDeclined: false,
              })}
              leader={
                <span
                  class={`${spriteLeaderClasses} sprite ${"texture-bubbles_white_1" satisfies TextureTailwindClass} ${"selectedMenuItem:texture-animated-bubbles_bounce_white" satisfies AnimatedTextureTailwindClass} sprites-normal-height zx:sprite-revert-to-white`}
                />
              }
              verticalAlignItemsCentre
            />
          </MenuItems>
          <div class="text-midGrey zx:text-zxWhite toppy:text-toppyGrey1 resHandheld:hidden">
            <QuitGameHint />
          </div>
        </div>
      </Dialog>
