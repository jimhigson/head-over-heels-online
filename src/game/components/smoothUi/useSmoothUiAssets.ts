...
import { maxBakedUpscaleFactor } from "../../render/filters/upscale/maxBakedUpscaleFactor";
import { loadHudFont, uiFontFamilies } from "../../render/text/uiFont";
import { importBakeUpscaledImageBlobOnce } from "./bakeUpscaledImageBlob.import";
...
const applySheet = async (
  ...
  if (existing?.factor !== factor) {
    const { bakeUpscaledImageBlob } = await importBakeUpscaledImageBlobOnce();
    const blob = await bakeUpscaledImageBlob(sourceUrl, factor);
 ...
 * upscaled blobs of their sheets. Everything is loaded dynamically -
...
export const useSmoothUiAssets = () => {
  ...
  const bakedUpscaleFactor = Math.min(
    Math.floor(totalUpscale),
    maxBakedUpscaleFactor(),
  );
  const smoothUi = smoothSprites && bakedUpscaleFactor >= 2;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await loadHudFont(smoothUi ? "smooth" : "base");
        if (cancelled) {
          return;
        }
        if (smoothUi) {
          ...
          await applySheet("blockStack", spritesSheetUrl, bakedUpscaleFactor);
          if (cancelled) {
            return;
          }
          if (spritesOptionName === "Toppy") {
            await applySheet("toppy", toppySheetUrl, bakedUpscaleFactor);
          }
  ...
  }, [rehydrated, smoothUi, bakedUpscaleFactor, spritesOptionName, dispatch]);
};
