...
import { bakeUpscaledTexture } from "../../render/filters/upscale/bakeUpscaledTexture";

/**
 * Upscale an image (a spritesheet the css shows sprites from) with the same
 * upscale bake the game engine gives its own spritesheets - the same call,
 ...
 * (see `bakeUpscaledImageBlob.import.ts`), so smoothing being off by default
 ...
 */
export const bakeUpscaledImageBlob = async (
  ...
  try {
    const baked = bakeUpscaledTexture(renderer, sourceTexture, factor);
    try {
