...
describe("jump grace", () => {
  const jumpingFixture: ItemsInTestRoomJson = {
    block_4: {
      type: "block",
      config: {
        style: "organic",
  ...
  test.for([["head"], ["heels"]] as const)(
    "%s doesn't levitate by sticking to the ceiling when jumping off a block and standing on nothing",
    ([characterName]) => {
      const gameState = setUpBasicGame({
        firstRoomItems: {
          ...jumpingFixture,
  ...
  test("head coming out of a gap can still jump to full height", () => {
    const gameState = setUpBasicGame({
      firstRoomItems: {
        ...jumpingFixture,
        head: {
  ...
  test("head coming out of a gap can still jump to full height coming off a volcano", () => {
    const gameState = setUpBasicGame({
      firstRoomItems: {
        startingBlock: {
          type: "block",
        ...
        // only way to get onto this volcano is to collect the shield
        // and then jump off the lower volcano
        topVolcano: {
          type: "deadlyBlock",
          config: {
            style: "volcano",
  ...
  test("can't continue jumping over a deadly block", () => {
    // this tests that the jump 'refresh' doesn't work along blocks that the player couldn't normally touch
    ...
    const startLives = headState(gameState).lives;
