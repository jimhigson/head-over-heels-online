...
export class E2EEventBus {
  ...
  /** an event whose payload is `undefined` is emitted by name alone, as in mitt */
  static emit<N extends E2EEventBusEventName>(
    name: undefined extends E2EEventBusPayloadMap[N] ? N : never,
  ): void;
  static emit<N extends E2EEventBusEventName>(
    name: N,
    payload?: E2EEventBusPayloadMap[N],
  ): void {
    E2EEventBus.singleton().emit(name, payload as E2EEventBusPayloadMap[N]);
  }
