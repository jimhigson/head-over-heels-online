...
const walkingImpl = <RoomId extends string, RoomItemId extends string>(
  ...
  const { inputStateTracker, currentCharacterName, gameTime } = gameState;
  // input is relative to the quarter angle the camera is visually closest to
  // - mid-turn this flips at the visual midpoint, not when the turn starts:
  const cameraAngle = currentQuarterCameraAngle(gameState);
  const {
    type,
    state: {
      action,
  ...
  const jumpInput: PressStatus =
    ...
    : "released";
  const inputVector = inputStateTracker.directionVector;
  const directionInput: Xyz =
    (
      // not the current character = no input
      !isCurrentCharacter ||
      // no input (the common idle frame): nothing to rotate, no allocation:
      xyzEqual(inputVector, originXyz)
    ) ?
      originXyz
      // the direction input is relative to the screen, so rotate it by the
      // inverse of the camera rotation to get a world-space walk direction:
    : rotateXyzByInverseCameraAngle(inputVector, cameraAngle);
