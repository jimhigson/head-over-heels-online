...
>(
  ...
  const mtvAvMagnitudeSquared = lengthXySquared(mtvAv);
  if (mtvAvMagnitudeSquared === 0) {
  ...
  const backingOffProjectedOnMovementVectorMagnitude: number =
    dotProductXyz(mtvOverlap, mtvAv) / mtvAvMagnitudeSquared;
  ...
  const unitM = unitVector(mtvAv);

  const slidingVel = scaleXyz(unitM, -moveSpeedPixPerMs.ball);

  if (log)
    console.log(`giving ${slidingItem.id} state.vels.sliding`, slidingVel);
...
>(
  ...
  if (d > 0) {
    ...
    if (log)
      console.log(`dot product zero, not stopping ${slidingItem.id} sliding`);
  }
  if (log) console.groupEnd();
};
