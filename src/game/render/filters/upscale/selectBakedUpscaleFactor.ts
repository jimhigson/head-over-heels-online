...
import { maxBakedUpscaleFactor } from "./maxBakedUpscaleFactor";
 ...
 * only by what the hardware will hold. 1 = no upscaling, ie the
 * plain art
 */
export const selectBakedUpscaleFactor = (state: GameRootState): number =>
  selectIsSmoothSprites(state) ?
    Math.min(Math.floor(selectTotalUpscale(state)), maxBakedUpscaleFactor())
  : 1;
