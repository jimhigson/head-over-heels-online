...
describe("subSpriteVariations", () => {
  ...
  test("creating a mix of animated and non-animated textures", () => {
    ...
    expect(
      ((animatedSprite as AnimatedSprite).textures as Texture[])[0].label,
    ).toEqual("dalek.1");
    expect(
      ((animatedSprite as AnimatedSprite).textures as Texture[])[1].label,
    ).toEqual("dalek.2");
  ...
  test("creating animated with {subSpriteVariations, times: undefined} gives a Sprite with the first texture", () => {
    ...
    expect(
      ((containerSprite as AnimatedSprite).textures as Texture[])[0].label,
    ).toEqual("dalek.1");
    expect(
      ((containerSprite as AnimatedSprite).textures as Texture[])[1].label,
    ).toEqual("dalek.2");
    expectTypeOf(containerSprite).toExtend<
      AnimatedSprite | Container<Sprite> | Sprite
    >();
