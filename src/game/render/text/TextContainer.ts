...
/**
 * The HUD web font, generated by `pnpm gen:font`. It is a pixel font built
 ...
 */
const hudFont = `${hudCharTextureSize.h}px "${uiFontFamilies.base}"`;
...
/**
 * Registers the HUD font and resolves once it is genuinely usable by canvas
 * 2d. The registration is made here rather than assumed: this is the only
 * module that rasterises with this family, so nothing else is obliged to have
 * asked for it first.
 */
export const loadHudFont = async (): Promise<void> => {
  await ensureUiFontLoaded("base");
};
 ...
 *
 * Callers must have awaited {@link loadHudFont}, or canvas 2d silently draws
 * with a fallback font; the {@link TextContainer} constructor throws if they
 * have not.
 */
