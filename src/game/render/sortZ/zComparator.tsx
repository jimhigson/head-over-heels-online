  ...
  switch (visualOverlap) {
    case OVERLAP: {
      ...
      if (renderBBsOrder === Z_COMPARATOR_OF_VISUALLY_OVERLAPPING_UNDECIDED) {
        // every spatial item has an entry (null = renders true to its
        // physical aabb), so only a non-null box gives a second, different
        // pair of boxes to test:
        const renderBBDifferentFromPhysical =
          aRenderBox !== null || bRenderBox !== null;
    ...
    case ADJACENT_X:
    case ADJACENT_Y: {
      // the classification names the gapped WORLD family (ADJACENT_X = gap on
      // the world-x family axis). The seam-handling bodies below are
      // camera-space; which camera family a world family renders as swaps
