...
/** the Map-shaped oracle graph's edges, in the same form the flat helper gives */
const oracleEdgeStrings = (graph: Graph<TestItem, boolean>): string[] =>
  [...graph]
...
const assertMatchesOracle = ({
  ...
  expect(zOrderGraphEdgeStrings(zEdges), frameLabel).toEqual(
    oracleEdgeStrings(oracle),
  );
