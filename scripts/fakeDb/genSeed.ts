 ...
 */
const campaignOwnerProvider: Provider = "github";
...
const userIdForProvider = (provider: Provider) =>
  provider === campaignOwnerProvider ?
    jimAtBlockstackingUserId
  : md5Uuid(provider);

const emailForProvider = (provider: Provider) => `${provider}@blockstack.ing`;

/** the auth.users + identity + profile rows that make one provider loginable */
const userSeedSql = (provider: Provider) => {
...
-- production, and creates one loginable local user per devAuthProviders so
...
${devAuthProviders.map(userSeedSql).join("\n")}
...
`;
  ...
  console.info(
    `local logins: ${devAuthProviders.map(emailForProvider).join(", ")} / ${devPassword}`,
  );
};
