...
export async function flattenFixture(
  ...
  const fixturePath = path.join(__dirname, "testFixtures", `${fixtureName}.ts`);

  // Create a temporary file to capture output
  const tempFile = path.join(__dirname, `temp-${Date.now()}.ts`);

  try {
    // Create a new project for each test with only the necessary files
    ...
    // flattenTypes now returns a string instead of logging
