 ...
 * the upscale algorithm's INTENT rather than its raster: boundary loops are
 * traced from the bitmap, then staircases whose steps are uniform 1:1, 2:1 or
 * 1:2 (the exact slopes the upscaler cuts) collapse to single diagonal segments.
...
/** the slopes the upscaler cuts, as (|dx|, |dy|) step shapes in subpixels */
 ...
 * one of the upscaler's 1:1, 2:1 or 1:2 slopes, lasting at least `minSteps`
 * whole steps. The diagonal is anchored at the MIDPOINTS of the run's first
 * and last sub-segments: that is where the upscaler's cut line actually
 ...
 * of different slopes meet (the upscaler's slope-transition raster): a segment
 ...
 * diagonal segment - ie where the geometry came from the upscaler's smoothing
