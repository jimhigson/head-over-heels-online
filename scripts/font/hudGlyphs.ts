...
/**
 * The y-offset of the char rows on the BlockStack spritesheet. The font
 * generator reads glyph pixels from a build-only strip cropped from these rows
 * (scripts/font/hudChars.webp, produced by scripts/iff2png.sh), so each glyph's
 * absolute frame.y is offset by this to index into the strip.
 */
export const charRowsTop = 993;
 ...
 * (the generator samples these pixels from the cropped char strip).
 */
