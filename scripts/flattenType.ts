...
import { normalizeTypeOutput } from "./schemaConversion/normalizeTypeOutput.js";
...
export class TypeFlattener {
  ...
  private buildJsonItemUnion(_depth: number): string {
    ...
    // If we couldn't find JsonItemType, check if this is a test fixture
    if (
      itemTypes.length === 0 &&
      this.currentSourceFile?.includes("testFixtures")
    ) {
      // Use simplified list for test fixtures
      itemTypes = ["door", "wall", "conveyor", "block"];
    }
