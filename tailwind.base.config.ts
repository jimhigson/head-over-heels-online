...
import { hideOsCursor } from "./src/tailwind/plugins/hideOsCursor";
...
/** classes in markdown can be encoded as urls like class1?class2&class3 */
export const extractClassesInMarkdown = {
  md(content: string) {
...
/**
 * everything the game, the level editor and the font editor share; each app's
 * own config spreads this and gives the content globs for its own sources
 */
export const tailwindBaseConfig = {
  ...
  theme: {
    ...
    spacing: {
      ...
      bottomStackPullup: `calc(var(--scale) * 20px)`,
