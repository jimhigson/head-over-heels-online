...
* for yes/no questions say yes/no only unless there is IMPORTANT and NON-OBVIOUS detail missing
 ...
 * **specs must import `test` from `./testUtils/test`, never from `@playwright/test`.** That module extends playwright's `test` with an automatic fixture that forwards the browser console into the node-side output for every test, so anything the game logs (notably the `[game-speed]` lines from visual-regression builds) reaches the playwright report and CI logs without each spec opting in. A spec importing `test` straight from `@playwright/test` still runs, but silently loses that forwarding - there is no playwright hook that reaches tests built on a different `test` object. `expect` and the types still come from `@playwright/test`.
